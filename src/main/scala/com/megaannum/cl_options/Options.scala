...
/*
  object Ref {
    def apply(provider: Options.Provider,
              names: List[Name], 
              help: String): Ref = new Ref(provider, names, help)

    def apply(provider: Options.Provider,
              name: Name, 
              help: String): Ref = new Ref(provider, List(name), help)

    def apply(provider: Options.Provider,
              name: String, 
              help: String): Ref = {
      val pp = new NameParser
      val r = pp.parseAll(pp.names, name)
      val names: List[Name] = r.get
      // This ignores if the opt has a value
      Ref(provider, names, help)

      // val optValues: List[Option.Value] = r.get
      // This ignores if the opt has a value
      // Ref(provider, optValues map {ov => ov.name} , help)
    }

    def apply(provider: Options.Provider,
              names: List[String], 
              help: String)(implicit d: DummyImplicit): Ref = 
      Ref(provider, names map {n => Name(n)}, help)
  }

  class Ref(val provider: Options.Provider,
            val name: Name, 
...
*/
/*
  abstract class Def(val ref: Ref) {
    var parentOptionsOp: SOption[Options] = None
    var childOptionDefs: List[Option.Def] = Nil

    def provider: Provider = ref.provider
    def name: String = ref.name
    def aliases: List[String] =ref.aliases 
    def help: String = ref.help

    def doMatch(other: Name): Boolean = other == name || aliases.exists(other)
